...
const NoteForm =()=>{
    ...
    const handleSubmit = async (e)=>{
        ...
        console.log(newNote);
        if(editing){
            console.log('http://localhost:3600/api/notes/'+selectedNote);
            await axios.put('http://localhost:3600/api/notes/'+selectedNote,newNote);
        }else{
            await axios.post('http://localhost:3600/api/notes',newNote)
        }
        window.location.href='/'
    }

    const getNote= async(id)=>{
        const res = await axios.get('http://localhost:3600/api/notes/'+id)
        console.log(res);
        setValues({
